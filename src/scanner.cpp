        ...
        yaml_mark_t start_mark, yaml_char_t **name)
{
    ...
    if (!STRING_INIT(parser, string, INITIAL_STRING_SIZE, yaml_char_t*)) goto error;
...
yaml_parser_scan_tag(yaml_parser_t *parser, yaml_token_t *token)
{
    ...
    if (CHECK_AT(parser->buffer, '<', 1))
    {
        ...
        handle = static_cast<yaml_char_t*>(yaml_malloc(1));
    ...
    else
    {
        ...
        else
        {
            ...
            yaml_free(handle);
            handle = static_cast<yaml_char_t*>(yaml_malloc(2));
            if (!handle) goto error;
            handle[0] = '!';
            handle[1] = '\0';
        ...
        int literal)
{
    ...
    if (!STRING_INIT(parser, string, INITIAL_STRING_SIZE, yaml_char_t*)) goto error;
    if (!STRING_INIT(parser, leading_break, INITIAL_STRING_SIZE, yaml_char_t*)) goto error;
    if (!STRING_INIT(parser, trailing_breaks, INITIAL_STRING_SIZE, yaml_char_t*)) goto error;
        ...
        int single)
{
    ...
    if (!STRING_INIT(parser, leading_break, INITIAL_STRING_SIZE, yaml_char_t*)) goto error;
    if (!STRING_INIT(parser, trailing_breaks, INITIAL_STRING_SIZE, yaml_char_t*)) goto error;
    if (!STRING_INIT(parser, whitespaces, INITIAL_STRING_SIZE, yaml_char_t*)) goto error;
