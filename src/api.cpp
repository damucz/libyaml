...
{
    yaml_char_t *new_start = static_cast<yaml_char_t*>(yaml_realloc(*start, (*end - *start)*2));
...
yaml_parser_initialize(yaml_parser_t *parser)
{
    ...
    if (!BUFFER_INIT(parser, parser->raw_buffer, INPUT_RAW_BUFFER_SIZE, unsigned char*))
        goto error;
    if (!BUFFER_INIT(parser, parser->buffer, INPUT_BUFFER_SIZE, yaml_char_t*))
        goto error;
...
yaml_string_read_handler(void *data, unsigned char *buffer, size_t size,
        size_t *size_read)
{
    yaml_parser_t *parser = static_cast<yaml_parser_t*>(data);
...
yaml_emitter_initialize(yaml_emitter_t *emitter)
{
    ...
    if (!BUFFER_INIT(emitter, emitter->buffer, OUTPUT_BUFFER_SIZE, yaml_char_t*))
        goto error;
    if (!BUFFER_INIT(emitter, emitter->raw_buffer, OUTPUT_RAW_BUFFER_SIZE, unsigned char*))
        goto error;
...
{
    yaml_emitter_t *emitter = static_cast<yaml_emitter_t*>(data);
        ...
        int implicit)
{
    ...
    if (version_directive) {
        version_directive_copy = static_cast<yaml_version_directive_t*>(yaml_malloc(sizeof(yaml_version_directive_t)));
        ...
        yaml_scalar_style_t style)
{
    ...
    value_copy = static_cast<yaml_char_t*>(yaml_malloc(length+1));
