    ...
    else
    {
            ...
            if (!*tag_handle) {
                ...
                for (tag_directive = parser->tag_directives.start;
                    ...
                    if (strcmp((char *)tag_directive->handle, (char *)tag_handle) == 0) {
                        ...
                        tag = static_cast<yaml_char_t*>(yaml_malloc(prefix_len+suffix_len+1));
                        if (!tag) {
        ...
        if (indentless_sequence && token->type == YAML_BLOCK_ENTRY_TOKEN) {
            ...
            if (token->type == YAML_SCALAR_TOKEN) {
                ...
                yaml_char_t *value = static_cast<yaml_char_t*>(yaml_malloc(1));
                if (!value) {
                    parser->error = YAML_MEMORY_ERROR;
                    goto error;
                }
        ...
        yaml_mark_t mark)
{
    ...
    value = static_cast<yaml_char_t*>(yaml_malloc(1));
    if (!value) {
        parser->error = YAML_MEMORY_ERROR;
        return 0;
    }
        ...
        yaml_tag_directive_t **tag_directives_end_ref)
{
            ...
            token->type == YAML_TAG_DIRECTIVE_TOKEN)
    {
        if (token->type == YAML_VERSION_DIRECTIVE_TOKEN) {
            ...
            version_directive = static_cast<yaml_version_directive_t*>(yaml_malloc(sizeof(yaml_version_directive_t)));
            if (!version_directive) {
                parser->error = YAML_MEMORY_ERROR;
                goto error;
            }
